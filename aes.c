...
static void add_round_key(uint8_t *state, const uint8_t *roundkeys) {
	*((uint64_t*)state) ^= *((uint64_t*)roundkeys);
	*((uint64_t*)state+1) ^= *((uint64_t*)roundkeys+1);
}
...
static void mix_columns(uint8_t *state) {
	...
	for (col = 0; col < 4; col++) {
		...
		state[4*col+0] = mul(temp[0], a[0]) ^ mul(temp[1], a[1]) ^     temp[2]        ^     temp[3];
		state[4*col+1] =     temp[0]        ^ mul(temp[1], a[0]) ^ mul(temp[2], a[1]) ^     temp[3];
		state[4*col+2] =     temp[0]        ^     temp[1]        ^ mul(temp[2], a[0]) ^ mul(temp[3], a[1]);
		state[4*col+3] = mul(temp[0], a[1]) ^     temp[1]        ^     temp[2]        ^ mul(temp[3], a[0]);
	}
}
...
static void inv_mix_columns(uint8_t *state) {
	...
	for (col = 0; col < 4; col++) {
		...
		temp[2] = state[4*col+2];
		temp[3] = state[4*col+3];
		state[4*col+0] = mul(temp[0], a[0]) ^ mul(temp[1], a[1]) ^ mul(temp[2], a[2]) ^ mul(temp[3], a[3]);
		state[4*col+1] = mul(temp[0], a[3]) ^ mul(temp[1], a[0]) ^ mul(temp[2], a[1]) ^ mul(temp[3], a[2]);
		state[4*col+2] = mul(temp[0], a[2]) ^ mul(temp[1], a[3]) ^ mul(temp[2], a[0]) ^ mul(temp[3], a[1]);
		state[4*col+3] = mul(temp[0], a[1]) ^ mul(temp[1], a[2]) ^ mul(temp[2], a[3]) ^ mul(temp[3], a[0]);
	}
}
