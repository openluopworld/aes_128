 ...
 * Suppose the mechine is little-endian.
 ...
 */
static const uint32_t IK0[256] = {
	       0x0,  0xb0d090e, 0x161a121c, 0x1d171b12, 
	...
	0x23d373ab, 0x28de7aa5, 0x35c961b7, 0x3ec468b9, 
	 0xfe75793,  0x4ea5e9d, 0x19fd458f, 0x12f04c81, 
	...
	0x1ed5ae3d, 0x15d8a733,  0x8cfbc21,  0x3c2b52f, 
	...
	0x1132f9ae, 0x1a3ff0a0,  0x728ebb2,  0xc25e2bc, 
	...
	0x1f6234d1, 0x146f3ddf,  0x97826cd,  0x2752fc3, 
	...
	0x10856342, 0x1b886a4c,  0x69f715e,  0xd927850, 
	0x64d90f0a, 0x6fd40604, 0x72c31d16, 0x79ce1418, 
	0x48ed2b32, 0x43e0223c, 0x5ef7392e, 0x55fa3020, 
	 0x1b79aec,  0xaba93e2, 0x17ad88f0, 0x1ca081fe, 
	...
	0x2264e947, 0x2969e049, 0x347efb5b, 0x3f73f255, 
	 0xe50cd7f,  0x55dc471, 0x184adf63, 0x1347d66d, 
...
void aes_encrypt_128(const uint8_t *roundkeys, const uint8_t *plaintext, uint8_t *ciphertext) {
    ...
    const uint32_t * rks = (uint32_t*)roundkeys;
    ...
    roundkeys += 160;
    ciphertext[0]  = SBOX[(uint8_t)t0]       ^ *roundkeys++;
    ciphertext[1]  = SBOX[(uint8_t)(t1>>8)]  ^ *roundkeys++;
    ciphertext[2]  = SBOX[(uint8_t)(t2>>16)] ^ *roundkeys++;
    ciphertext[3]  = SBOX[t3>>24]            ^ *roundkeys++;
    ciphertext[4]  = SBOX[(uint8_t)t1]       ^ *roundkeys++;
    ciphertext[5]  = SBOX[(uint8_t)(t2>>8)]  ^ *roundkeys++;
    ciphertext[6]  = SBOX[(uint8_t)(t3>>16)] ^ *roundkeys++;
    ciphertext[7]  = SBOX[t0>>24]            ^ *roundkeys++;
    ciphertext[8]  = SBOX[(uint8_t)t2]       ^ *roundkeys++;
    ciphertext[9]  = SBOX[(uint8_t)(t3>>8)]  ^ *roundkeys++;
    ciphertext[10] = SBOX[(uint8_t)(t0>>16)] ^ *roundkeys++;
    ciphertext[11] = SBOX[t1>>24]            ^ *roundkeys++;
    ciphertext[12] = SBOX[(uint8_t)t3]       ^ *roundkeys++;
    ciphertext[13] = SBOX[(uint8_t)(t0>>8)]  ^ *roundkeys++;
    ciphertext[14] = SBOX[(uint8_t)(t1>>16)] ^ *roundkeys++;
    ciphertext[15] = SBOX[t2>>24]            ^ *roundkeys++;
...
void aes_decrypt_128(const uint8_t *roundkeys, const uint8_t *ciphertext, uint8_t *plaintext) {    
    ...
    t2  =  ct[2] ^ *( ((uint32_t*)roundkeys) + 42);
    t3  =  ct[3] ^ *( ((uint32_t*)roundkeys) + 43);

    /*
     * 9 rounds of decryction
     */
    roundkeys += 144;
    for (i = 1; i < AES_ROUNDS; ++i) {
    ...
    plaintext[3]  = INV_SBOX[t1>>24]            ^ *roundkeys++;
    plaintext[4]  = INV_SBOX[(uint8_t)t1]       ^ *roundkeys++;
    plaintext[5]  = INV_SBOX[(uint8_t)(t0>>8)]  ^ *roundkeys++;
    plaintext[6]  = INV_SBOX[(uint8_t)(t3>>16)] ^ *roundkeys++;
    plaintext[7]  = INV_SBOX[t2>>24]            ^ *roundkeys++;
    plaintext[8]  = INV_SBOX[(uint8_t)t2]       ^ *roundkeys++;
    plaintext[9]  = INV_SBOX[(uint8_t)(t1>>8)]  ^ *roundkeys++;
    plaintext[10] = INV_SBOX[(uint8_t)(t0>>16)] ^ *roundkeys++;
    plaintext[11] = INV_SBOX[t3>>24]            ^ *roundkeys++;
    plaintext[12] = INV_SBOX[(uint8_t)t3]       ^ *roundkeys++;
    plaintext[13] = INV_SBOX[(uint8_t)(t2>>8)]  ^ *roundkeys++;
    plaintext[14] = INV_SBOX[(uint8_t)(t1>>16)] ^ *roundkeys++;
